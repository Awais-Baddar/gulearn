...
export async function signup(req, res) {
  ...
  try {
    ...
    try {
      ...
      console.log("error in stream user creation ");
    }
...
export async function login(req, res) {
  try {
    ...
    if (!isPassCorrect)
      return res.send(401).json({ message: "incorect email or password " });
...
try {
    ...
    {
       return res.status(401).json({message:"all fields are required",missingFields:[
...
image:updatedUser.profilePic||""
    ...
    console.log(`stream users updated for ${updatedUser.fullName}`)
} catch (streamError) {
    
    console.log('error in stream updating ')
}
